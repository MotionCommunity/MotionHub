...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let res: Response;
  try {
    res = await fetch(`${leagueApiBaseUrl()}/v1/matches/${id}`, {
      method: "PATCH",
  ...
  if (!res.ok) {
    return NextResponse.json({ error: body?.error || "Failed to update match" }, { status: res.status });
  }
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let res: Response;
  try {
    res = await fetch(`${leagueApiBaseUrl()}/v1/matches/${id}`, { cache: "no-store" });
  } catch {
    return NextResponse.json(
      { error: "League API is offline. Start league-api on port 4001." },
      { status: 502 }
  ...
  const body = await readJsonBody<{ item?: unknown; error?: string }>(res);
  if (!res.ok) {
    return NextResponse.json({ error: body?.error || "Failed to load match" }, { status: res.status });
  }
  return NextResponse.json({ item: body?.item });
}
