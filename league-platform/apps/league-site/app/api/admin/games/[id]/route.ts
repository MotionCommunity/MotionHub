...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  let res: Response;
  try {
    res = await fetch(`${leagueApiBaseUrl()}/v1/games/${encodeURIComponent(id)}`, { method: "DELETE" });
  } catch {
  ...
  if (!res.ok) {
    return NextResponse.json({ error: body?.error || "Failed to delete game" }, { status: res.status });
  }
