...
export async function GET() {
  ...
  let res: Response;
  try {
    res = await fetch(`${leagueApiBaseUrl()}/v1/players`, { cache: "no-store" });
  } catch {
  ...
  if (!res.ok) {
    return NextResponse.json({ error: body?.error || "Failed to load players" }, { status: res.status });
  }
...
export async function POST(req: Request) {
  ...
  let res: Response;
  try {
    res = await fetch(`${leagueApiBaseUrl()}/v1/players`, {
      method: "POST",
  ...
  if (!res.ok) {
    return NextResponse.json({ error: body?.error || "Failed to create player" }, { status: res.status });
  }
