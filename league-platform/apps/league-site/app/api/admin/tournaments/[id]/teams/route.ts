...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  let res: Response;
  try {
    res = await fetch(`${leagueApiBaseUrl()}/v1/tournaments/${encodeURIComponent(id)}/teams`, {
      cache: "no-store"
    });
  } catch {
  ...
  if (!res.ok) {
    return NextResponse.json({ error: body?.error || "Failed to load teams" }, { status: res.status });
  }
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!res.ok) {
    return NextResponse.json({ error: body?.error || "Failed to create team" }, { status: res.status });
  }
