...
// Storage: local motion-tournament-sync.json (no GitHub dependency).
...
const DEFAULT_STATE = {
  ...
  matchResults: [],
  scheduledMatches: [],
  // Live/published bracket config used by stats + master hub
  bracketConfig: null,
  // Draft bracket tree (updated by bot + staff; master can publish to bracketConfig)
  bracketDraft: null,
  recentChanges: {},
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const data = await readJson(FILE, DEFAULT_STATE);
      return res.status(200).json({
        ...
        bracketDraft: data.bracketDraft ?? DEFAULT_STATE.bracketDraft,
  ...
  if (req.method === 'POST') {
    ...
    try {
      const current = await readJson(FILE, { ...DEFAULT_STATE });
      ...
      if (!current.bracketDraft || typeof current.bracketDraft !== 'object') current.bracketDraft = DEFAULT_STATE.bracketDraft;
      ...
      const next = {
        ...
        bracketDraft: body.bracketDraft !== undefined ? body.bracketDraft : current.bracketDraft,
